...
though the upstream tarball is reachable. Affected formulae set
`GOPROXY=direct` and `GOSUMDB=off` in the brew sandbox to bypass the
proxy for module fetches.

Applies to every cli-guard consumer formula in this tap (currently
[Formula/ward.rb](../Formula/ward.rb)).
